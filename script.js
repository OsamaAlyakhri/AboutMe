...
setTimeout(_ => { if (!site_loaded) for (element of document.getElementsByClassName(`waitload`) || []) element.innerHTML = `${element.innerHTML}<center><h3 onclick=\"refresh()\" style=\"color: #FEFEFE; white-space: nowrap; padding-top: 200px; cursor: pointer; font-size: 12px;\"> Taking too long to load? Click here! </h3></center>` }, 10 * 1000)

window.onload = _ => {
  site_loaded = true
  for (element of document.getElementsByClassName(`waitload`) || []) setTimeout(_ => Object.mergify(element.style, { opacity: 0, pointerEvents: `none`, transitionDuration: `1s` }), 1000)
  document.getElementById(`load`).innerHTML = `Site took <p style=\"color: #E18499; display: inline;\">${new Date().getTime() - init}</p>ms to load`
}

let [guilds, members, lastUpdated, invervalID] = [localStorage.getItem(`kurasad-guilds`), localStorage.getItem(`kurasad-members`), localStorage.getItem(`kurasad-lastUpdated`)]
...
function run(guilds, member, lastUpdated) {
  ...
  invervalID = setInterval(_ => since.innerHTML = stringify(), 1000)
}
...
async function update() {
  ...
  localStorage.setItem(`osama-guilds`, body.guilds)
  localStorage.setItem(`osama-members`, body.members)
  localStorage.setItem(`osama-lastUpdated`, new Date().getTime())
...
let reset = _ => {
  if (confirm(`Are you sure you want to reset site data?`)) {
    localStorage.removeItem(`osama-guilds`)
    localStorage.removeItem(`osama-members`)
    localStorage.removeItem(`osama-lastUpdated`)
    update()
    alert(`Site data reset!`)
  }
}

let escape = _ => document.getElementById(`menu`).checked = false

let refresh = _ => window.location.reload()
